...
export function summarise(before, after) {
  ...
  const attached = (after.receptorSources || []).filter(e => !beforeEdges.has(edgeKey(e)));
  const restatused = (after.receptorSources || []).filter(e => beforeEdges.has(edgeKey(e)) && beforeEdges.get(edgeKey(e)) !== e.status);
  ...
  if (restatused.length) {
    ...
    out.push(verified === restatused.length && verified
