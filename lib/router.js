    ...
    },
    {
      ...
      handler: (req, res, m) => {
        ...
        const source = row.source_id != null
          ? { authors: row.authors, year: row.year, title: row.title, journal: row.journal, pmid: row.pmid, doi: row.doi }
          : null;
        json(res, 200, {
          id: row.id,
          label: row.label,
      ...
      handler: (req, res, m) => {
        ...
        const out = rows.map(row => ({
          ...
          source: row.source_id != null
            ? { authors: row.authors, year: row.year, title: row.title, pmid: row.pmid, doi: row.doi }
            : null,
        }));
        json(res, 200, out);
      },
