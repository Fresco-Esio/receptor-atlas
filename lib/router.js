...
import { reviewDrift, cabinetBinding, ledgerClinical, archiveNarrative, rollupStatus, receptorStatuses, receptorSources } from './queries.js';
...
export function apiRoutes(db) {
  ...
  // Task 5 / Citation redesign: receptor row + its full source list (assembled
  ...
  // Task 6: receptors that appear in a given volume — the primary source edge
  // only (rs.is_primary = 1), so a volume keeps showing exactly one citation
  // per receptor even though a receptor may now have many attached sources.
  ...
  // Task 9 / Citation redesign: sources library (read + write). `kind` ('article'
  ...
  return [
    ...
    },
    {
      ...
      handler: (req, res, m) => {
        ...
        const out = rows.map(row => ({
          ...
          source: row.source_id == null ? null : toCitation({
            id: row.source_id, authors: row.authors, year: row.year, title: row.title, pmid: row.pmid, doi: row.doi,
          }, { journal: false }),
        }));
        json(res, 200, out);
      },
    ...
    // Task 11: persist review state.
    {
      ...
      handler: async (req, res, m) => {
        ...
        // Task 17: a review touch stamps last_reviewed_at on every section of this
    ...
    // Task 19: structured volume data shaped for the volume's own render code.
    {
      method: 'GET',
